...
class Module07 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  case class PassengerCar(val name: String = "Passenger car", val carries: String = "people") extends Car {}

  // concrete class CargoCar should subclass Car, set the name field "Cargo car" and
  // the carries field to "cargo"

  case class CargoCar(val name: String = "Cargo car", val carries: String = "cargo") extends Car {}
  ...
  abstract class Engine extends RollingStock {
    ...
    val cars = new mutable.ListBuffer[Car]()

    def pull: String = name + " pulls " + cars.mkString(" and ")

    // def add(vehicle: Car): Unit = if (cars.length < maxCars) cars.append(vehicle) else throw new IllegalStateException("Cars limited exceeded")

    def add(car: Car): Engine = {
      if (cars.size < maxCars) cars += car
      else throw new IllegalStateException("Number of cars exceeding the maximum allowed number of cars")
      this
    }
  ...
  case class SteamEngine(val name: String = "Steam engine", val maxCars: Int = 3) extends Engine {}
  ...
  case class DieselEngine(val name: String = "Diesel engine", val maxCars: Int = 6) extends Engine {}
  ...
  case class ShuntEngine(val name: String = "Shunt engine", val maxCars: Int = 10) extends Engine {
  ...
  object Engine {
    val shunt = new ShuntEngine
    val diesel = new DieselEngine
    val steam = new SteamEngine
  }
  
  object Car {
    val cargo = new CargoCar
    val passenger = new PassengerCar
  }
