...
class Module08 extends KoanSuite with Matchers with SeveredStackTraces {
  ...
  koan("Fun with top types of values") {
    ...
    lots.isInstanceOf[AnyRef] should be (true) // Huh? Why?
  ...
  def listProduct(p: Product): List[Any] = p.productIterator.toList
   ...
   koan("make a list from any product") {
    ...
    listProduct(p1) should be (List("Sally", "Smith", 43, 'F'))
  }
